...
function bank_playerLogin( oldAccount, currentAccount )
	...
	if isGuestAccount( oldAccount ) == true and getElementType( source ) == "player" then
		...
		if player_balance then
			...
			outputChatBox( "Saldo del banco: $ " .. tostring( playersAccount[ source ].balance ), source, 255, 255, 0 )
		else
			playersAccount[ source ] = nil;
...
function bank_playerLogout( oldAccount, currentAccount )
	if isGuestAccount( currentAccount ) == true and isGuestAccount( oldAccount ) ~= true and getAccountName( oldAccount ) == playersAccount[ source ]:accountname() then
		...
		outputChatBox( "Has terminado tu sesion satisfactoriamente. Su saldo bancario ha sido guardado!", source, 0, 255, 0 )
	end
end
	...
	if command == "deposit" then
		if isPlayerInBank( player ) then
			if getPlayerMoney( player ) >= money_int then
					...
					outputChatBox( "tu saldo actual es $".. tostring( playersAccount[ player ].balance )..".", player, 255, 255, 0 )
					outputChatBox( "Dinero ($".. tostring( money_int ) ..") depositado correctamente.", player, 255, 255, 0 )
					takePlayerMoney( player, money_int )
			else
				outputChatBox( "Fondos insuficientes.", player, 250, 0, 0 )
			end
		else
			outputChatBox( "Debes estar en el marcador de banco para usar este comando.", player )
		end	
	elseif command == "withdraw" then
		if isPlayerInBank( player ) then
			if playersAccount[ player ].balance < money_int then
				outputChatBox( "Fondos insuficientes.", player )
			else
                bank_savePlayerMoney( player, getPlayerAccount( player ) )
				outputChatBox( "Has retirado $"..tostring( money_int )..".", player, 255, 255, 0 )
...
addCommandHandler( "retirar", bank_withdrawDepositCommand )
addCommandHandler( "depositar", bank_withdrawDepositCommand )
...
function withdrawMoney( player, money )
	...
	if type( money ) == 'number' and playersAccount[ player ].balance < money then
		triggerClientEvent( player, "bank_showWarningMessage", player, "Fondos insuficientes!" )
	elseif type( money ) == 'string' and money == 'all' then
		...
		if money > 0 then
			...
			triggerClientEvent( player, "bank_showWarningMessage", player, "No tienes dinero en tu\ncuenta" )
		end
    else
		if money > 0 then
			local atm = ( banksInfo[ playerBankID ].ATM and true or false )
...
function depositMoney( player, money )
    ...
    elseif type( money ) == 'string' and money == 'all' then
		...
		if money == 0 then 
			triggerClientEvent( player, "bank_showWarningMessage", player, "No tienes dinero para depositar" )
			return
		end
		local atm = ( banksInfo[ playerBankID ].ATM and true or false )
		local triggered = triggerEvent( "onPlayerDepositMoney", player, getPlayerBank( player ), money, atm )
